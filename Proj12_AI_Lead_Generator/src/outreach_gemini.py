"""
outreach_gemini.py — Outreach Email Generator using Gemini
...
async def generate_outreach_email(lead: Dict, preview: Dict) -> Dict:
    """
    Generate a personalized outreach email as a structured JSON object using Gemini.
    """
...
"""
    ...
    for model_name in models_to_try:
        try:
            logger.info(f"Attempting outreach email generation for {lead['name']} with Gemini model: {model_name}")
            ...
            if json_match:
                ...
                logger.warning(f"Could not parse JSON from outreach model {model_name} for {lead['name']}. Using raw text for body.")
                return {"subject": f"A quick question about {lead['name']}", "body": response_text}
        except Exception as e:
            logger.warning(f"Model {model_name} failed during email generation for {lead['name']}: {e}")
            last_exception = e

    logger.error(f"All Gemini models failed for email generation for {lead['name']}. Last error: {last_exception}")
    return {"subject": "Error", "body": f"Error generating email: {last_exception}"}
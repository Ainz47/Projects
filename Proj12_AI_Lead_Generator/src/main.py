"""
main.py — Main Orchestrator for WebPulse Lead Generation
...
from evaluator_gemini import evaluate_website
...
log_path = LOG_DIR / f"pipeline_run_gemini_{run_id}.log"
...
async def run_pipeline(category: str, city: str, state: str, max_results: int = 10):
    ...
    """
    logger.info(f"Starting GEMINI pipeline for {category} in {city}, {state}")
    ...
    output_file = output_dir / f"leads_gemini_{category.replace(' ', '_')}_{city}_{state}_{run_id}.csv"
    ...
    json_file = output_dir / f"leads_gemini_{category.replace(' ', '_')}_{city}_{state}_{run_id}.json"
    ...
    summary_file = LOG_DIR / f"run_summary_gemini_{run_id}.json"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="WebPulse Lead Generation Pipeline (Gemini Version)")

...
function fieldSheet(state, actions, blocks) {
  ...
  rows.push(el('div', { class: 'row' }, [
    el('button', {
      ...
      text: block.campaignSlot ? 'Talk slot' : 'Make this the talk slot',
...
export function render(state, actions) {
  ...
  const banner = missing
    ? el('p', { class: 'error', text: 'This day has no talk slot. Open a block and make it the slot before saving.' })
    : null;
